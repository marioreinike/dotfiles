---
name: banana-2
description: "Creative-Director image generation via Google Gemini Nano Banana (Flash 3.1/2.5). Invoke ONLY via /banana-2-2 — does not auto-trigger because each call costs money. Use when you want the full prompt-engineering pipeline (5-component formula, domain modes, aspect-ratio + imageSize control, batch, brand presets, cost tracking) instead of the lightweight /banana-2. Keywords - banana-2, creative director, prompt engineering, domain mode, batch, preset."
disable-model-invocation: true
argument-hint: "[generate|edit|chat|inspire|batch|setup|preset|cost] <idea, path, or command>"
allowed-tools: Bash, Read, Write, Edit
metadata:
  version: "1.4.1-fork"
  upstream: "AgriciDaniel/banana-2-claude"
  mcp-package: "@ycse/nanobanana-mcp (not installed — use Python fallback in scripts/)"
---
...
|---------|-------------|
| `/banana-2` | Interactive -- detect intent, craft prompt, generate |
| `/banana-2 generate <idea>` | Generate image with full prompt engineering |
| `/banana-2 edit <path> <instructions>` | Edit existing image intelligently |
| `/banana-2 chat` | Multi-turn visual session (character/style consistent) |
| `/banana-2 inspire [category]` | Browse prompt database for ideas |
| `/banana-2 batch <idea> [N]` | Generate N variations (default: 3) |
| `/banana-2 setup` | Install MCP server and configure API key |
| `/banana-2 preset [list\|create\|show\|delete]` | Manage brand/style presets |
| `/banana-2 cost [summary\|today\|estimate]` | View cost tracking and estimates |
...
For `/banana-2 edit`, Claude should also enhance the edit instruction:
...
## Prompt Inspiration (`/banana-2 inspire`)
...
## Batch Variations (`/banana-2 batch`)

For `/banana-2 batch <idea> [N]`, generate N variations:
...
| Error | Resolution |
|-------|-----------|
| MCP not configured | Run `/banana-2 setup` |
...
This fork runs **without the MCP server** — the Python fallback scripts call Gemini's REST API directly.

Prerequisites:
- `GEMINI_API_KEY` exported in the environment (already in `~/.zshrc`; same key the lightweight `/banana` skill uses).
- Python 3 (stdlib only — no pip deps).
- ImageMagick (optional, only for the post-processing recipes).

If `GEMINI_API_KEY` is unset when the user invokes `/banana-2`, stop and tell them — do not try to proceed.

Skip `scripts/setup_mcp.py` and `scripts/validate_setup.py`; both target the MCP setup we are deliberately not using here.

## Tool Routing (Important — MCP is NOT installed)

The upstream SKILL.md assumes an MCP server (`@ycse/nanobanana-mcp`) is available. In this fork, **always use the Python fallback scripts**:

| Upstream MCP tool | Use instead |
|---|---|
| `gemini_generate_image` | `python3 "${CLAUDE_SKILL_DIR}/scripts/generate.py" --prompt "..." --aspect-ratio "16:9" [--model gemini-2.5-flash-image] [--resolution 2K]` |
| `gemini_edit_image` | `python3 "${CLAUDE_SKILL_DIR}/scripts/edit.py" --image PATH --prompt "..."` |
| `gemini_chat` | Not available without MCP — run sequential `generate.py` calls, preserve descriptive anchors manually |
| `set_aspect_ratio` / `set_model` | Pass as `--aspect-ratio` / `--model` flags to `generate.py` |
| `get_image_history` / `clear_conversation` | Not available — there is no MCP session state |

Where the upstream SKILL.md mentions `set_aspect_ratio`, `set_model`, `gemini_generate_image`, etc., translate to the corresponding `generate.py` / `edit.py` flag.
